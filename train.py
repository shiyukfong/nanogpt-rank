...
optimizer_name = 'adamw' # 'adamw', 'sgd', or 'muon'
wandb_project = 'owt'
...
model_args = dict(n_layer=n_layer, n_head=n_head, n_embd=n_embd, n_block=n_block, 
                  bias=bias, vocab_size=None) # start with model_args from command line
...
if init_from == 'scratch':
    ...
    total_tokens_processed = checkpoint.get('total_tokens_processed', iter_num * tokens_per_iter)
...
optimizers = model.configure_optimizers(
    ...
    rank_analysis_mode=rank_analysis_mode
)
...
def estimate_loss():
    ...
    for split in ['train', 'val']:
        ...
        # synchronize loss across all ranks in DDP
        if ddp:
...
def get_lr(it):
    ...
    if it > lr_decay_iters:
        return min_lr
    if warmup_iters <= it < warmdown_iters:
        return learning_rate
    # 3) in between, use cosine decay down to min learning rate
    decay_ratio = (it - warmdown_iters) / (lr_decay_iters - warmdown_iters)
...
def _make_scheduler(opt):
    if decay_lr:
        return torch.optim.lr_scheduler.LambdaLR(
            opt, lr_lambda=lambda it: (get_lr(it) / learning_rate), last_epoch=iter_num - 1
        )
    return torch.optim.lr_scheduler.LambdaLR(opt, lr_lambda=lambda _: 1.0, last_epoch=iter_num - 1)

schedulers = [_make_scheduler(opt) for opt in optimizers]
...
while True:
    ...
    # All ranks participate in eval to avoid DDP desynchronization
    if iter_num % eval_interval == 0:
        losses = estimate_loss()
        if master_process:
            print(f"step {iter_num}: train loss {losses['train']:.4f}, val loss {losses['val']:.4f}, tokens {total_tokens_processed:,}")
            if wandb_log:
                wandb.log({
                    "iter": iter_num,
    ...
    if iter_num == 0 and eval_only:
        print("eval_only is set, exiting after the first evaluation")
        break
    ...
    iter_num += 1
    local_iter_num += 1
    total_tokens_processed += tokens_per_iter
